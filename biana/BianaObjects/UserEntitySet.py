...
class UserEntitySet(object):
	...
	def getTagLinkageDegree(self, tag):
	    ...
	    return self.get_nodes_by_linker_degree_cutoff(seed_set, linker_degree_cutoff)
	...
	def get_nodes_by_linker_degree_cutoff(self, seed_set, linker_degree_cutoff):
            """
            Get nodes having a linker degree equal or higher to a given cutoff.
            """
            result_set = set()
            #print seed_set
	...
	def has_relation(self, user_entity_id1, user_entity_id2):
		return self.network.has_edge(user_entity_id1, user_entity_id2)


	def select_user_entity_relations_between_two_subsets(self, user_entity_ids_list1, user_entity_ids_list2, types_list=None):
		"""
		...
		# TO CHECK. IT IS NOT A GOOD IDEA IN THIS CASE SELECT EDGES BY relation ID... BECAUSE THEN THIS RELATION MAY INVOLVE MULTIPLE EDGES!

		print "255811 BETWEEN SET 1: ", 255811 in user_entity_ids_list1, " SET 2: ", 255811 in user_entity_ids_list1
		print "260545 BETWEEN SET 1: ", 260545 in user_entity_ids_list2, " SET 2: ", 260545 in user_entity_ids_list2

		if types_list is not None:
			raise ValueError("select_user_entity_relations_between_two_subsets does not accept any more the parameter \"types_list\"")
		
		selected_relations = []

		for current_1 in user_entity_ids_list1:
			for current_2 in user_entity_ids_list2:
				if self.network.has_edge(current_1, current_2):
					selected_relations.extend(self.network.get_edge(current_1, current_2))
