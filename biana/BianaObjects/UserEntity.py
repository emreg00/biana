...
class UserEntity(object):
    ...
    def __init__(self, id, listExternalEntityId=None, dbAccess=None):
        """
        externalEntities
        """
    ...
    def get_attribute(self, attribute_identifier):
        """
        Returns a set with the values for this attribute
        """

        eE_dict = self.dbAccess.get_external_entities_dict( externalEntityIdsList = self.setExternalEntityId,
                                                            attribute_list = [attribute_identifier] )

        return_set = sets.Set()

        for eE in eE_dict.values():
            return_set.update(eE.get_attribute(attribute_identifier))
