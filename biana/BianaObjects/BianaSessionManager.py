...
class BianaSessionManager(object):
    ...
    def get_user_entity(self, user_entity_id):
        ...
        if not self.dictUserEntity.has_key(user_entity_id):
            ...
            objUserEntity = UserEntity.UserEntity(user_entity_id, listIdExternalEntity, self.dbAccess )
    ...
    def expand_user_entity_set(self, user_entity_set_id, is_last_level=False):
        ...
        try:
			...
			self.outmethod("<user_entity id=\"%s\" type=\"%s\"/>" %(idUserEntity2, partner_type))
                    ...
                    if inserted:
                        #print (idUserEntity1, idUserEntity2, externalEntityRelationID, relation_type, partner_type)
                        self.outmethod("<user_entity_relation node1=\"%s\" node2=\"%s\" type=\"%s\" relation_id=\"%s\"/>" %(idUserEntity1, idUserEntity2, relation_type, externalEntityRelationID))
                OutBianaInterface.send_end_process_message()
    ...
    def create_network(self, user_entity_set_id, level=0, include_relations_last_level = True, relation_type_list=[], relation_attribute_restriction_list=[], use_self_relations=True, expansion_attribute_list=[], expansion_relation_type_list=[], expansion_level=2, attribute_network_attribute_list=[], group_relation_type_list=[]):
        """
        ...
        try:
            ...
            # Javi added: transform restrictions (for transferred attributes)
            relation_attribute_restriction_list = self.dbAccess.transform_expanded_attribute_restrictions(relation_attribute_restriction_list)
            ...
            OutBianaInterface.send_error_notification( message = "Error while creating network", error = traceback.format_exc() )
    ...
    def output_external_entity_relation_details(self, out_method=None, external_entity_relation_id_list=[], node_attributes=[], relation_attributes=[], participant_attributes=[], output_format="xml"):
        """
	...
	#columns.extend([ "Participant 2 %s" %x for x in participant_attributes])
        ...
        if output_format == "xml":
            ...
            out_method( output_utilities.get_tabulated_table( columns = columns, values = values) )

        return



    def get_defined_node_attributes(self, user_entity_set_id, user_entity_id, attribute, output_1_value_per_attribute, return_set=False, substitute_node_attribute_if_not_exists=False, output_only_unique_values=False):
        """
        ...
        defined_attributes = self.get_defined_node_attributes_helper(user_entity_set_id, user_entity_id, attribute, output_1_value_per_attribute, output_only_unique_values)
        ...
        if substitute_node_attribute_if_not_exists:
            ...
            while len(defined_attributes)==0:
                ...
                defined_attributes = self.get_defined_node_attributes_helper(user_entity_set_id, user_entity_id, current_attribute, output_1_value_per_attribute, output_only_unique_values)
    ...
    def get_defined_node_attributes_helper(self, user_entity_set_id, user_entity_id, attribute, output_1_value_per_attribute, output_only_unique_values):
        """
	...
	user_entity_set = self.dictUserEntitySet[user_entity_set_id]
        ...
        attribute_values_dict = self.dbAccess.get_user_entity_attributes( unification_protocol_name = self.unification_protocol_name,
									  ...
									  only_uniques = output_only_unique_values )
	...
	output_only_unique_values = If true, for each attribute, shows only values associated with "unique" type
        """
...
##                                                                                            attribute_identifier = current_attribute )
        ...
        for current_node in user_entity_id_list:
            ...
            for current_attribute in attributes:
                ...
                attribute_values = self.get_defined_node_attributes(user_entity_set.id, current_node, current_attribute, output_1_value_per_attribute, substitute_node_attribute_if_not_exists = substitute_node_attribute_if_not_exists, return_set = True, output_only_unique_values = output_only_unique_values)
	...
	f_method(user_entity_set = user_entity_set,
		 ...
		 output_only_unique_values = output_only_unique_values)

        OutBianaInterface.send_end_process_message()


    def output_external_entity_details(self, out_method=None, attributes=[], user_entity_id_list=[], output_format="xml"):
        """
	...
	output_only_unique_values = If true, for each attribute, shows only values associated with "unique" type
        ...
        if output_format == "xml":
            ...
            out_method( output_utilities.get_tabulated_table( columns = columns, values = [columns]) )
	...
	#default_ids_dict.update(self.dbAccess.get_default_external_entity_ids( externalEntityIDsList=eE_dict.keys() ))
        ...
        for current_node in user_entity_id_list:
            ...
            eE_dict = self.dbAccess.get_external_entities_dict( externalEntityIdsList = uEobj.get_externalEntitiesIds_set(), 
                                                                attribute_list = attributes, only_uniques = output_only_unique_values )
	    default_ids_dict = dict([ (x,"-") for x in eE_dict.keys() ])
	    default_ids_dict.update(self.dbAccess.get_default_external_entity_ids( externalEntityIDsList=eE_dict.keys() ))
    ...
    def output_user_entity_set_sequences_in_fasta(self, user_entity_set_id, only_selected = False, out_method = None, type="proteinsequence", attributes = [], include_tags_info=True, one_line_per_sequence=False, output_1_value_per_attribute=False, output_only_native_values=False, output_only_unique_values = False):
        """
	...
	if output_only_native_values is True or output_1_value_per_attribute is True:
	    ...
	    OutBianaInterface.send_error_notification(message="Output Error!" ,error="Displaying only single value feature is not available in FASTA formatted output. Use Native/All")
	    return
	...
	default_ids_dict.update(self.dbAccess.get_default_external_entity_ids( externalEntityIDsList=list_eE_to_search_dict.keys() ))
        ...
        for current_eE_id, uE_id in list_eE_to_search_dict.iteritems():
            ...
            current_eE = self.dbAccess.get_external_entities_dict( externalEntityIdsList = [current_eE_id], attribute_list = new_attributes, only_uniques = output_only_unique_values )[current_eE_id]
