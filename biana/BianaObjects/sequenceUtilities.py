...
def get_sequence_mutual_equivalence_lists(alignment):
    """
    Returns two lists of mutual equivalences between the two proteins in the alignment
    """

    if alignment.nseqs != 2:
        raise ValueError("Trying to get mutual sequence equivalences from an alignment of more than 2 proteins")

    first_alignned_seq = alignment.get_aligned_sequence(seq_pos=0)
    second_alignned_seq = alignment.get_aligned_sequence(seq_pos=1)
    
    length = len(first_alignned_seq)
    ...
    for x in xrange(length):
        if second_alignned_seq[x]=='-': 
            gaps_aln2 += 1
            seq1_pos +=1 
            equivalences1.append(None)
            continue
        if first_alignned_seq[x]=='-':
            equivalences2.append(None)
            gaps_aln1 += 1
            seq2_pos += 1
            continue
        equivalences1.append(seq2_pos)
        equivalences2.append(seq1_pos)
        seq1_pos += 1
        seq2_pos += 1

    return (equivalences1, equivalences2)


def get_sequence_binary_equivalence_lists(alignment, gap_percentage_cutoff=100.0):
    """
    Given an alignment of two proteins, gives two mask lists
    """

    #alignment.print_alignment()

    if alignment.nseqs != 2:
        raise ValueError("Trying to get sequence equivalences from an alignment of more than 2 proteins")
    ...
    #print first_alignned_seq
    #print
    #print second_alignned_seq
    ...
    for x in xrange(length):
        if second_alignned_seq[x]=='-': 
            gaps_aln2 += 1
            equivalences1.append(0)
            continue
        if first_alignned_seq[x]=='-':
            gaps_aln1 += 1
            equivalences2.append(0)
            continue
        equivalences1.append(1)
        equivalences2.append(1)


    if gaps_aln1*100/length > gap_percentage_cutoff or gaps_aln2*100/length > gap_percentage_cutoff:
        print first_alignned_seq
        print second_alignned_seq
        raise ValueError("Sequence equivalences is greater than cutoff %s" %(gap_percentage_cutoff))
    

    return (equivalences1, equivalences2)


def binary_equivalence_list_to_fragments_list(binary_equivalences_list):

    fragments = []
    current_start = None
    
    for x in xrange(len(binary_equivalences_list)):
        if binary_equivalences_list[x]==1 and current_start is None:
            current_start = x
            continue
        if binary_equivalences_list[x]==0 and current_start is not None:
            fragments.append((current_start,x-1))
            current_start = None
            continue
        
    if current_start is not None:
        fragments.append((current_start,len(binary_equivalences_list)-1))
...
def blast_fasta_file(fasta_file, output_file):
    """
    Test to avoid:
    Traceback (most recent call last):
    File "select_sequences.py", line 26, in <module>
    blast_results = sequenceUtilities.self_blast_fasta_file( fasta_file="temp/"+fields[0], fd_output_file=blast_output )
    File "/home/jgarcia/phD_project/BIANA_PROJECT/biana/biana/src/biana/BianaObjects/sequenceUtilities.py", line 533, in self_blast_fasta_file
    if os.fork():
    ...
    command = "%s -i %s -p blastp -d %s -z 1720800858 -F F -v 0 -o %s" %(biana_globals.BLASTALL_EXEC, fasta_file, fasta_file, output_file)

    os.system(command)

    blast_out_fd = open(output_file,'r')
    blast_results = parse_blastall_output(blast_out_fd)
    blast_out_fd.close()
...
def self_blast_fasta_file(fasta_file, fd_output_file):
    ...
    if os.fork():
        ...
        try:
            if os.fork():
                # Still the same child
                for x in xrange(len(sequences_list)):
                    os.write(p_writefd, ">%s\n%s\n\n" %(ids_list[x],sequences_list[x]))
                os._exit(0)
            else:
                # Grandchild
                try:
