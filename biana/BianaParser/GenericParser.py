	...
	participant_re = re.compile("(.+):\s*(.+)") #("(\w[(\s\w)]*):\s()")
        ...
        for line in self.input_file_fd:
			...
			if x in self.mandatory_columns:
				...
				ref_type = self.default_ref_type
                                if attribute_identifier[0] in self.first_char_to_ref_type:
				    ref_type = self.first_char_to_ref_type[attribute_identifier[0]]
				    attribute_identifier = attribute_identifier[1:] 
                                if attribute_identifier.lower()=="proteinsequence":
                                    current_value = ProteinSequence(current_value)
                                if attriubte_identifier.lower() in self.attribute_identifier_to_max_value_length:
                                    self.verify_attribute_length(attribute_identifier.lower(), current_value)
                                new_external_entity.add_attribute( ExternalEntityAttribute( attribute_identifier= attribute_identifier, 
                                                                                            value=current_value, 
                                                                                            type=ref_type) ) #type="cross-reference") )
