...
public class SelectUserEntityAttributesSelection extends CommandPanel implements ActionListener {
    ...
    public SelectUserEntityAttributesSelection(Vector<String> pAvailableAttributes, String pSetName, BianaProcessController controller) {
	...
	if( e.getActionCommand()=="add_attribute_list" || e.getActionCommand()=="add_attribute_restriction" || e.getActionCommand()=="add_negative_attribute_restriction" ){
		...
		for( int i=0; i<lines.length; i++ ){
			...
			if( e.getActionCommand()=="add_attribute_list" ){
			    this.values.add("(\""+this.getJList().getSelectedValue()+"\",\""+lines[i].trim()+"\")");
			}
			else if( e.getActionCommand()=="add_attribute_restriction" ){
			    this.restrictions.add("(\""+this.getJList().getSelectedValue()+"\",\""+lines[i].trim()+"\")");
			}
			else if( e.getActionCommand()=="add_negative_attribute_restriction" ){
			    this.negative_restrictions.add("(\""+this.getJList().getSelectedValue()+"\",\""+lines[i].trim()+"\")");
			}
		    }
		}
		if( buffer.length()>0 ){
	...
	else if( e.getActionCommand()=="ontology_navigate" ){
	    //new OntologyDialog(this.controller.getParentFrame(), controller.getOntologyTree("PSI_MI_OBO"), this.getJTextArea());
	    System.err.println("Trying to open ontology");
	    JTree tree = controller.getOntologyTree("taxonomy");
	    if( tree != null ){
		new OntologyDialog(this.controller.getParentFrame(), controller.getOntologyTree("taxonomy"), this.getJTextArea(), "taxonomy");
	    }
	...
	return "select_user_entities_using_attributes( identifier_description_list = ["+Utilities.join(this.values,",")+"], external_entity_attribute_restriction_list=["+Utilities.join(this.restrictions,",")+"], id_type=\"embedded\", user_entity_set_id=\""+this.setName+"\", clear_previous_selection = True, negative_external_entity_attribute_restriction_list=["+Utilities.join(this.negative_restrictions,",")+"])";
    }
